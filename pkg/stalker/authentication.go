...
// new one will be issued by stalker portal, reserved and Stalker's config will be updated.
func (p *Portal) handshake() error {
	...
	req, err := http.NewRequest("GET", p.Location+"server/load.php?type=stb&action=handshake&prehash=0&token="+p.Token+"&JsHttpRequest=1-xml", nil)
	if err != nil {
		return err
	}
...
func (p *Portal) authenticate() (err error) {
	...
	content, err := p.httpRequest(p.Location + "server/load.php?type=stb&action=do_auth&login=" + p.Username + "&password=" + p.Password + "&device_id=" + p.DeviceID + "&device_id2=" + p.DeviceID2 + "&JsHttpRequest=1-xml")
